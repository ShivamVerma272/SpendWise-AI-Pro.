...
const r = Router();
const token = u => jwt.sign({ id: u.id, name: u.name, email: u.email }, process.env.JWT_SECRET, { expiresIn: '7d' });
...
r.post('/register', async (req, res) => {
  try {
    ...
    const u = { id: id(), name: String(name).trim(), email: normalized, password: await bcrypt.hash(password, 10), createdAt: new Date().toISOString() };
    db.users.push(u);
    await writeDb(db);
